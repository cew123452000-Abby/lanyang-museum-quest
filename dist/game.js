...
const immersiveSceneData=[
  ...
  {image:'assets/rpg-scene-02-mist-forest-v1.jpg',alt:'蘭陽博物館迷霧森林展示中的退冰痕跡、上升山徑與臺灣山毛櫸葉',objects:[
    {name:'退冰遺痕',item:'氣候變暖紀錄',clue:'冰河期結束後，整體氣候逐漸回暖。',x:83,y:18,key:'warm'},
    {name:'上升山徑',item:'高海拔路標',clue:'怕熱的生物會往較高、較涼爽的地方移動。',x:37,y:53,key:'up'},
    {name:'金黃葉標本',item:'山毛櫸記憶葉',clue:'臺灣山毛櫸是展場可查證的冰河孑遺植物。',x:7,y:36,key:'beech'}],
    decoys:[{name:'山間雲霧',msg:'雲霧是森林環境的一部分，但不是這條氣候遷移因果鏈。',x:66,y:29},{name:'蕨類',msg:'先找會說明「變暖與往高處移動」的證據。',x:31,y:84},{name:'展場霧燈',msg:'這是展場照明，不能當成自然史證據。',x:6,y:14}],
  ...
  {image:'assets/rpg-scene-03-forest-council-v1.jpg',alt:'山林永續議會場景中的祖靈規範、母獸足跡與有限森林資源',objects:[
    ...
    {name:'母獸與幼獸足跡',item:'繁殖守護證詞',clue:'不捕捉懷孕母獸，能保留族群繁殖的機會。',x:38,y:68,key:'birth'},
    {name:'伐木年輪樁',item:'資源有限警鐘',clue:'森林材木與獵物不是取之不盡；過度利用會讓循環中斷。',x:84,y:61,key:'limit'}],
    decoys:[{name:'華麗獵具',msg:'工具精美不等於使用方式永續。',x:68,y:29},{name:'野花',msg:'花朵很醒目，但法庭需要能約束行為的證據。',x:5,y:59},{name:'原住民空背簍',msg:'空背簍本身不能證明採集有節制。',x:66,y:39}],
  ...
  {image:'assets/rpg-scene-05-kuroshio-v1.jpg',alt:'黑潮觀測站中的深藍水樣、暖流溫度計、北向洋流與海漂果實',objects:[
    {name:'深藍水樣瓶',item:'深藍清澈讀數',clue:'黑潮並不是黑色；展項呈現深藍、較清澈的水色。',x:62,y:75,key:'color'},
    {name:'暖流溫度計',item:'較暖水溫讀數',clue:'黑潮水溫相對較高。',x:74,y:58,key:'warm'},
    {name:'北向流標',item:'向北流向標',clue:'黑潮沿臺灣東側往北流動。',x:22,y:21,key:'north'},
    {name:'漂流果實',item:'海漂旅行證物',clue:'穗花棋盤腳果實能藉海流漂送，鰻魚也會利用海流洄游。',x:13,y:50,key:'travel'}],
    decoys:[{name:'貝殼',msg:'貝殼無法校正黑潮的四項讀數。',x:15,y:75},{name:'繩圈',msg:'繩圈是航海工具，不是海流方向證據。',x:6,y:85},{name:'舊羅盤',msg:'羅盤只告訴方位；請找真正指向北方的洋流標記。',x:21,y:83}],
...
function enablePointerDrag(sourceSelector,dropSelector,getPayload,onDrop){
  $$(sourceSelector).forEach(source=>{
    source.draggable=false;
    source.addEventListener('pointerdown',startEvent=>{
      if(startEvent.pointerType==='mouse'&&startEvent.button!==0)return;
      const pointerId=startEvent.pointerId,startX=startEvent.clientX,startY=startEvent.clientY,payload=getPayload(source);
      let moved=false,ghost=null,over=null;
      const clearOver=()=>{over?.classList.remove('drag-over');over=null};
      ...
      const onMove=e=>{
        ...
        if(!moved){moved=true;source.dataset.dragged='true';source.classList.add('dragging');ghost=source.cloneNode(true);ghost.removeAttribute('id');ghost.removeAttribute('data-object');ghost.removeAttribute('data-assembly-token');ghost.className='drag-ghost';ghost.setAttribute('aria-hidden','true');document.body.append(ghost)}
        e.preventDefault();moveGhost(e);const target=document.elementFromPoint(e.clientX,e.clientY)?.closest(dropSelector);if(target!==over){clearOver();over=target;over?.classList.add('drag-over')}
      };
      const finish=e=>{
        if(e.pointerId!==pointerId)return;
        source.removeEventListener('pointermove',onMove);source.removeEventListener('pointerup',finish);source.removeEventListener('pointercancel',finish);source.classList.remove('dragging');
        const target=moved?document.elementFromPoint(e.clientX,e.clientY)?.closest(dropSelector):null;clearOver();ghost?.remove();
        if(target)onDrop(target,payload);if(moved)setTimeout(()=>delete source.dataset.dragged,0)
      };
      source.setPointerCapture?.(pointerId);source.addEventListener('pointermove',onMove,{passive:false});source.addEventListener('pointerup',finish);source.addEventListener('pointercancel',finish)
    })
  })
}
...
function renderRoute(q){
  const items=['龍骨水車','鴨母船割稻','竹圍','洗衫棚'];let selected=null;const placed={};
  $('#puzzle').innerHTML=`<h2>把四件展示拖到現場地圖的正確位置</h2><p class="route-help">桌機可用滑鼠、平板與手機可用手指直接拖曳；也可先點選右側物件，再點地圖虛線框。地圖取自「蘭博蒐查線」現場配置。</p><div class="route-workspace"><div class="museum-map" id="museumMap">${items.map((x,i)=>`<button class="map-drop" data-slot="${i}" aria-label="${x}的位置"><span>放到<br>${20+i} 號</span></button>`).join('')}</div><div class="object-tray" id="objectTray">${items.map((x,i)=>`<button class="drag-object" data-object="${i}" aria-label="拖曳或點選${x}"><span class="object-art object-${i}" aria-hidden="true"></span><span>${x}</span></button>`).join('')}</div></div><div class="answer-row"><button class="secondary-btn" id="clearRoute">全部取回</button><button class="primary-btn" id="checkRoute">核對地圖</button></div>`;
  const paint=()=>{$$('[data-object]').forEach(b=>{const id=+b.dataset.object;b.classList.toggle('selected',selected===id);b.classList.toggle('placed',Object.values(placed).includes(id))});$$('[data-slot]').forEach(z=>{const s=+z.dataset.slot,id=placed[s];z.classList.toggle('filled',id!==undefined);z.innerHTML=id===undefined?`<span>放到<br>${20+s} 號</span>`:`<span class="object-art object-${id}" aria-label="${items[id]}"></span>`})};
  const assign=(slot,id)=>{for(const k in placed)if(placed[k]===id)delete placed[k];placed[slot]=id;selected=null;paint();tone(470)};
  $$('[data-object]').forEach(b=>b.onclick=()=>{if(b.dataset.dragged)return;selected=+b.dataset.object;paint()});
  $$('[data-slot]').forEach(z=>z.onclick=()=>{if(selected!==null)assign(+z.dataset.slot,selected)});
  enablePointerDrag('[data-object]','[data-slot]',b=>+b.dataset.object,(z,id)=>assign(+z.dataset.slot,id));
  $('#clearRoute').onclick=()=>{Object.keys(placed).forEach(k=>delete placed[k]);selected=null;paint()};
  $('#checkRoute').onclick=()=>items.every((_,i)=>placed[i]===i)?complete(q):feedback(false,'還有物件放錯位置。請對照現場地圖紅色編號 20～23。');paint()
}
...
function shuffledIndexes(length){
  ...
  if(indexes.length>1&&indexes.every((value,i)=>value===i))[indexes[0],indexes[1]]=[indexes[1],indexes[0]];
  return indexes
}
function renderImmersiveAssembly(q){
  ...
  if(!Array.isArray(scene.tokenOrder)||scene.tokenOrder.length!==r.objects.length)scene.tokenOrder=shuffledIndexes(r.objects.length);
  const tokenOrder=scene.tokenOrder;let active=null;save();
  $('#puzzle').innerHTML=`<section class="assembly-lab"><div class="rpg-phase"><span class="active">1 搜查完成</span><span class="active">2 操作線索</span><span>3 打開封印</span></div><div class="assembly-head"><div><p class="eyebrow">證據操作臺</p><h2>${q.id===0?'重建建築設計鏈':q.id===1?'接回冰河記憶路徑':q.id===2?'修復山林永續契約':q.id===3?'疊回水系時間圖層':'校正黑潮觀測儀'}</h2><p>可用滑鼠或手指把下方道具拖到位置；也可先點道具、再點位置。每個位置都要能說出證據理由。</p></div><button class="secondary-btn" id="backToScene">回場景查證</button></div><div class="evidence-workbench"><div class="assembly-slots">${r.slots.map((s,i)=>`<button data-assembly-slot="${i}"><small>${i+1}</small><b>${s[0]}</b><span>放入線索</span></button>`).join(q.id===1||q.id===3?'<i>→</i>':'')}</div><div class="assembly-tray">${tokenOrder.map(i=>{const o=r.objects[i];return `<button class="assembly-token" data-assembly-token="${i}" aria-label="拖曳或點選${o.item}"><span>◆</span><b>${o.item}</b><small>${o.clue}</small></button>`}).join('')}</div></div><div class="answer-row"><button class="secondary-btn" id="clearAssembly">全部取回</button><button class="primary-btn" id="checkAssembly">啟動封印</button></div></section>`;
  ...
  const placeToken=(slot,token)=>{for(const k in scene.assembly)if(scene.assembly[k]===token)delete scene.assembly[k];scene.assembly[slot]=token;active=null;save();paint();tone(530)};
  $$('[data-assembly-token]').forEach(b=>b.onclick=()=>{if(b.dataset.dragged)return;active=+b.dataset.assemblyToken;paint();tone(440)});
  $$('[data-assembly-slot]').forEach(b=>b.onclick=()=>{if(active===null)return feedback(false,'先從下方調查袋選一件線索道具。');placeToken(+b.dataset.assemblySlot,active)});
  enablePointerDrag('[data-assembly-token]','[data-assembly-slot]',b=>+b.dataset.assemblyToken,(slot,token)=>placeToken(+slot.dataset.assemblySlot,token));
